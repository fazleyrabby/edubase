...
@section('title', "$district->name District — ILMATLAS")

@section('content')
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="mb-8">
        <a href="{{ route('locations.division', $district->division) }}" class="text-sm text-indigo-600 hover:text-indigo-800">&larr; {{ $district->division->name }} Division</a>
    </div>
        ...
        <div class="mb-8">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($upazilas as $upazila)
                    <a href="{{ route('locations.upazila', $upazila) }}" class="px-4 py-2 bg-gray-100 rounded-lg text-sm text-gray-700 hover:bg-indigo-100 hover:text-indigo-700">{{ $upazila->name }}</a>
                @endforeach
            </div>
        </div>
    @endif
